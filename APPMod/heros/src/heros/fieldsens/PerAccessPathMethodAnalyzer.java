...
public class PerAccessPathMethodAnalyzer<Field, Fact, Stmt, Method> {
	...
	private final Fact sourceFact;
	private final AccessPath<Field> accessPath;
	private final Map<WrappedFactAtStatement<Field,Fact, Stmt, Method>, WrappedFactAtStatement<Field,Fact, Stmt, Method>> reachableStatements = Maps.newHashMap();
	private final List<WrappedFactAtStatement<Field, Fact, Stmt, Method>> summaries = Lists.newLinkedList();
	private final Context<Field, Fact, Stmt, Method> context;
	private final Method method;
	private final DefaultValueMap<FactAtStatement<Fact, Stmt>, ReturnSiteResolver<Field, Fact, Stmt, Method>> returnSiteResolvers = new DefaultValueMap<FactAtStatement<Fact, Stmt>, ReturnSiteResolver<Field,Fact,Stmt,Method>>() {
		@Override
	...
	private final DefaultValueMap<FactAtStatement<Fact, Stmt>, ControlFlowJoinResolver<Field, Fact, Stmt, Method>> ctrFlowJoinResolvers = new DefaultValueMap<FactAtStatement<Fact, Stmt>, ControlFlowJoinResolver<Field,Fact,Stmt,Method>>() {
		@Override
	...
	private final CallEdgeResolver<Field, Fact, Stmt, Method> callEdgeResolver;
	private final PerAccessPathMethodAnalyzer<Field, Fact, Stmt, Method> parent;
	private final Debugger<Field, Fact, Stmt, Method> debugger;
	...
	public void scheduleUnbalancedReturnEdgeTo(WrappedFactAtStatement<Field, Fact, Stmt, Method> fact) {
		...
		resolver.addIncoming(new WrappedFact<Field, Fact, Stmt, Method>(fact.getWrappedFact().getFact(), fact.getWrappedFact().getAccessPath(), 
				fact.getWrappedFact().getResolver()), null, Delta.empty());
	}
	...
	private class Job implements Runnable {
		...
		private final WrappedFactAtStatement<Field, Fact, Stmt, Method> factAtStmt;
