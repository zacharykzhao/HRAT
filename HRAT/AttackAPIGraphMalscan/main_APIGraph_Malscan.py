...
from APIGraph_Utils import load_train_data, load_all_test_data, trans2triple_rw, degree_centrality_extraction, katz_feature, \
    get_new_sen_idx, find_2nd_nn_l2
from APIGraphmodel import DQN
from APIGraph_env import CFGModifierEnvConstraints
...
if __name__ == '__main__':
    # load data
    train_data_file = 'APIGraph_train_o.pkl'
    test_data_file = 'APIGraph_test_o.pkl'
    save_path = './results'
    node_map_path = '/Volumes/Samsung8702/testAPIGraph/malware/node_relation'
    constraints_path = "/Volumes/158.132.255.220/apk/testAPK_nodesAndCons/malware_cons_0308"

    if not os.path.exists(save_path):os.makedirs(save_path)
    #
    X_train_am_ori, X_train_sen_idx_ori, y_train_ori, X_train_degree_cen_ori = load_train_data(train_data_file)

    X_test_sha256_all, X_test_am_all, X_test_sen_idx_all = load_all_test_data(test_data_file)
    seq_save_path = os.path.join(save_path, 'seq')
    ...
    for zidx in range(len(X_test_sha256_all)):
        ...
        file_name1 = seq_save_path + "/" + X_test_sha256 + "action_list" + str(0) + ".txt"
        if os.path.exists(file_name1):
            continue
        node_transition_file = os.path.join(node_map_path, X_test_sha256 + ".npz")
        node_map = sparse.load_npz(node_transition_file)

        X_test_am = X_test_am_all[zidx]
        print("get triple set  ", X_test_sha256)
        triple_path = os.path.join(save_path,'triple_set')
        ...
        sen_idx = get_new_sen_idx(X_test_sen_idx, node_map)

        y_test = 0
        X_test_degree_cen = degree_centrality_extraction(X_test_am, sen_idx, node_map)
        katz_fea = katz_feature(X_test_am, sen_idx, node_map)
        X_test_degree_cen = np.append(X_test_degree_cen, katz_fea)
        print(X_test_sha256)
        node_number = X_test_am.shape[0]
        Q_fea = X_test_degree_cen
        ...
        env = CFGModifierEnvConstraints(X_test_triple, y_test,
        ...
        while True:
            ...
            action_type = dqn.choose_action(state, actions_num=actions_num, EPSILON=epison)
            # action_type = 3
            ...
            # reward = env.getReward(env.cur_graph)
