...
from APIGraph_Utils import find_nn_torch
...
class CFGModifierEnvConstraints(object):
    ...
    def __init__(self, target_graph, label,
        ...
        self.node_map = np.array(node_map.todense())
        self.sen_map = sen_map
        self.X_train = X_train
        self.y_train = y_train
        self.train_set = train_set
        self.train_label = train_label
        self.enhanced_sen_idx = target_sen_api_idx
    ...
    def step(self, action, k=1):
        self.enhanced_sen_idx = self.get_new_sen_idx(self.sen_api_idx)
        ...
        self.state = self.getDegreeCentrality(self.cur_graph, self.enhanced_sen_idx)
        katz_fea = self.getDegreeCentrality(self.to_adjmatrix(self.cur_graph), self.enhanced_sen_idx)
    ...
    def reset(self):
        self.enhanced_sen_idx = self.get_new_sen_idx(self.sen_api_idx)
        self.state = self.getDegreeCentrality(self.adj_sparse, self.enhanced_sen_idx)
        katz_fea = self.katz_feature_torch(self.to_adjmatrix(self.adj_sparse), self.enhanced_sen_idx)
        self.state = torch.cat((self.state, np.squeeze(katz_fea)), 0)
    ...
    def get_new_sen_idx(self, sen_idx):
        n_ori = self.node_map.shape[1]
        ...
        new_sam = np.matmul(idx_matrix,
                            self.node_map.transpose())  # n_sen * n_node_new, 每行应该表示原每个sensitive api，对应的新的node list

        # sen_map : 每行应表示，每个新的senapi包含的老sensitive api的位置
        #
        new_idx_m = np.matmul(self.sen_map.todense(), new_sam)
    ...
    def getDegreeCentrality(self, adj, sen_api_idx):
        idx_matrix = np.zeros((len(sen_api_idx), self.node_map.shape[0]))
        ...
        # tmp = np.array(self.node_map.todense())
        tmp = self.node_map
        tmp = torch.from_numpy(tmp).to(device)
        # adj_dense = self.node_map.dot(adj_dense).dot(self.node_map.T)
        adj_dense = torch.matmul(tmp.float(), torch.matmul(adj_dense.float(), tmp.float().T))
        adj_dense[adj_dense > 1] = 1
    ...
    def del_node(self, graph):
        ...
        # 调整node map
        self.node_map = np.delete(self.node_map, tar_node, axis=1)
    ...
    def add_node(self, graph):
        ...
        # 调整node map
        to_add_row = np.zeros((1, self.node_map.shape[1]))
        self.node_map = np.row_stack((self.node_map, to_add_row))
        to_add_col = np.zeros((self.node_map.shape[0], 1))
        to_add_col[-1] = 1
        self.node_map = np.column_stack((self.node_map, to_add_col))
    ...
    def katz_feature_torch(self, graph, sen_api_idx, alpha=0.1, beta=1.0, normalized=True):
        n = self.node_map.shape[0]
        # tmp = np.array(self.node_map.todense())
        tmp = self.node_map
        tmp = torch.from_numpy(tmp).to(device)
        graph_new = torch.matmul(tmp.float(), torch.matmul(graph.float(), tmp.float().T))
        # graph_new = self.node_map.dot(graph).dot(self.node_map.transpose())
    ...
    def get_gradient(self, graph):
        ...
        triple_torch = Variable(tmpz, requires_grad=True)
        feature = self.getDegreeCentrality(triple_torch, self.enhanced_sen_idx).to(device)

        densegraph = self.to_adjmatrix(triple_torch)
        feature_katz = self.katz_feature_torch(densegraph, self.enhanced_sen_idx).to(device)
